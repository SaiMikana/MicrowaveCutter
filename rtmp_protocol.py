...
class RtmpClient:
    ...
    def handshake(self):
        ...
        #print('c1', hexlify(c1.payload))
        self.stream.read_uchar()
        s1 = rtmp_protocol_base.Packet()
        s1.decode(self.stream)
        #print('s1 first', s1.first)
        #print('s1 second', s1.second)
        self.handshake_key = s1.payload[-32:];
        #print('s1', len(s1.payload), hexlify(s1.payload))
    ...
    def handle_message_pre_connect(self, msg):
        ...
        #print("Got pre connect message", msg)
        ...
        if msg['msg'] == DataTypes.COMMAND:
            ...
            #print("WINDOW_ACK_SIZE! %d" % msg['window_ack_size'])
            self.writer.write(msg)
            self.writer.flush()
            
            #print('handshake_key', hexlify(self.handshake_key));
            swf_key = unhexlify('de9a94ca4a1779a28bf661bcd0cdc58f816fa0ede1422c46c8faf1787622108f')
            #print('swf_key', hexlify(swf_key));
            dig = hmac.new(self.handshake_key, msg=swf_key, digestmod=hashlib.sha256).digest()
            
            ed = pack('>BB I I 32s', 0x01, 0x01, 21770, 21770, dig)
            self.writer.write({'msg': 4, 'event_type': 27, 'event_data': ed})
            self.writer.flush()
        elif msg['msg'] == DataTypes.SET_PEER_BANDWIDTH:
            #assert msg['window_ack_size'] == 2500000, msg
            #assert msg['limit_type'] == 2, msg
            #print("SET_PEER_BANDWIDTH! %d" % msg['limit_type'])
            pass
        elif msg['msg'] == DataTypes.USER_CONTROL:
            #print("USER_CONTROL!")
            if msg['event_type'] == 26:
                msg['event_type'] = 27
            return False
    ...
    def handle_simple_message(self, msg):
        ...
        if msg['msg'] == DataTypes.SET_PEER_BANDWIDTH:
            #print('Handle SET_PEER_BANDWIDTH')
            msg['msg'] = DataTypes.WINDOW_ACK_SIZE
            msg['window_ack_size'] = 50000
            self.writer.write(msg)
            self.writer.flush()
            return True
